...
class DRISHYAMTLModel(nn.Module):
    ...
    """
    def __init__(self, backbone_name='tu-tf_efficientnet_b4.ns_jft_in1k', pretrained=True, num_classes=5, num_masks=4, use_coral=False):
        super().__init__()
        ...
        if use_coral:
            # CORAL head: outputs K-1 binary logits for ordinal rank
            self.classifier = nn.Sequential(
                nn.Dropout(0.2),
                nn.Linear(encoder_out_dim, num_classes - 1)
            )
            # Monotonically decreasing biases for rank consistency
            with torch.no_grad():
                self.classifier[1].bias.data = torch.linspace(1.0, -1.0, num_classes - 1)
        else:
            # Standard head: outputs K logits
            self.classifier = nn.Sequential(
                nn.Dropout(0.2),
